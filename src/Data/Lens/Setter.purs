...
conjOver :: forall s t a. (BooleanAlgebra a) => Setter s t a a -> a -> s -> t
...
addModifying :: forall s a m. (MonadState s m, Semiring a) => SetterP s a -> a -> m Unit
addModifying p = modifying p <<< add

mulModifying :: forall s a m. (MonadState s m, Semiring a) => SetterP s a -> a -> m Unit
...
conjModifying :: forall s a m. (MonadState s m, BooleanAlgebra a) => SetterP s a -> a -> m Unit
conjModifying p = modifying p <<< flip conj

appendModifying :: forall s a m. (MonadState s m, Semigroup a) => SetterP s a -> a -> m Unit
